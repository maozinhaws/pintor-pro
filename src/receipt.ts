...
import { toast, ptFloat } from './utils';
import { calcOrcTotal } from './rooms';
...
// ── Abrir/Fechar modal ───────────────────────────────────────────
export function abrirModalRecibo(i: number): void {
  _reciboOrcIdx = i;
  const o = S.orcs[i];
  if (!o) return;
...
// ── Gerar PDF do recibo ──────────────────────────────────────────
export function gerarReciboPDF(): void {
  ...
  let valor = custom
    ...
    : tot;
  if (valor <= 0) {
    toast('<svg class="ico" aria-hidden="true"><use href="#ico-alert"/></svg> Informe um valor válido.');
    return;
  }
  ...
  const data = {
    ...
    empresa: { nome: (cfg as any).empresa || 'Prestador', cnpj: (cfg as any).doc || '', tel: (cfg as any).tel || '', email: (cfg as any).emailEmpresa || '', end: (cfg as any).endEmpresa || '', logo: (cfg as any).logo || '' },
    ...
    pgto,
    obs,
    sigPintor: (cfg as any).assinatura || '',
  };
...
export function gerarReciboHTML(d: {
  id: string;
  dataRecebimento: string;
  empresa: { nome: string; cnpj: string; tel: string; email: string; end: string; logo: string };
  cliente: { nome: string; doc: string; tel: string; end: string };
  pagador?: { nome: string; tel: string; end: string } | null;
  descServicos: string;
  valor: number;
  pgto: string;
  obs: string;
  ...
  const recebidoDe = hasPagador ? d.pagador!.nome : d.cliente.nome;
  const recebidoDoc = hasPagador ? '' : (d.cliente.doc ? `, portador(a) do CPF/CNPJ <strong>${d.cliente.doc}</strong>,` : '');
  const pagadorHtml = hasPagador ? `
    <div class="section">
      <div class="section-title">Dados do Pagador</div>
      <div class="info-grid">
        <div class="info-item"><span class="info-lbl">Nome</span><span class="info-val">${d.pagador!.nome}</span></div>
        ${d.pagador!.tel ? `<div class="info-item"><span class="info-lbl">Telefone</span><span class="info-val">${d.pagador!.tel}</span></div>` : ''}
        ${d.pagador!.end ? `<div class="info-item" style="grid-column:1/-1"><span class="info-lbl">Endereço</span><span class="info-val">${d.pagador!.end}</span></div>` : ''}
      </div>
    </div>` : '';
